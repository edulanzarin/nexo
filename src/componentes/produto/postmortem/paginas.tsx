...
import { FormularioPM } from "./formulario";
import { ListaGestao } from "./lista-gestao";
import { ListaMinha } from "./lista-minha";

/**
 * As três páginas do Post Mortem, escritas UMA vez. Cada módulo de setor tem só
 * os `page.tsx` que o Next exige, e eles delegam para cá com o próprio id de
 * módulo; o setor sai daí. Sem isto seriam quatro cópias das mesmas páginas, e
 * a quinta (quando entrar um setor novo) sairia diferente das outras.
 *
 * A tranca é daqui, no servidor, lendo as regras de `postmortem-acesso`:
 * navegação no cliente não refaz o layout, e a tela esconder botão não é
 * permissão.
 */
...
export async function PaginaMinha({ modulo }: { modulo: ModuloId }) {
  ...
  return <ListaMinha modulo={modulo} />;
}
...
export async function PaginaGestao({ modulo }: { modulo: ModuloId }) {
  ...
  return <ListaGestao modulo={modulo} />;
}

/**
 * O relatório. O caminho carrega a seção de ONDE se entrou, que é o que o
 ...
 *
 * Relatório de outro setor não existe neste caminho: 404, e não "sem
 * permissão", porque quem não pode ver não descobre que ele existe.
 */
