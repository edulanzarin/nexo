...
import { COR_MOVIMENTO } from "@/componentes/produto/pessoal/cores";
import { ModalFicha } from "@/componentes/produto/pessoal/ficha-funcionario";
import {
  FaixaFiltrosPessoal,
  resumoFiltros,
  useSelecaoPessoal,
} from "@/componentes/produto/pessoal/filtros-pessoal";
import { ModalPessoas, type DimensaoDrill, type Drill } from "@/componentes/produto/pessoal/modal-pessoas";
import {
  PainelMovimentacoes,
  vistaEhEfetivo,
  type VistaPessoas,
} from "@/componentes/produto/pessoal/painel-movimentacoes";
import { PainelQuebraTurnover, pctTurnover } from "@/componentes/produto/pessoal/quebra-turnover";
import { SerieTurnover } from "@/componentes/produto/pessoal/serie-turnover";
import { emAnos } from "@/componentes/produto/pessoal/tempo-casa";
...
import { buscarJson, useConsulta, useEmpresas } from "@/hooks/use-consulta";
...
/**
 * Rotatividade de uma empresa: quem entrou e saiu no período sobre o efetivo,
 * contra o período anterior, e onde a troca se concentra (setor, cargo,
 * horário, perfil). Os filtros da faixa recortam a tela inteira e valem só
 * para a empresa em que foram escolhidos.
 *
...
export default function Conteudo() {
  ...
  const empresa = p.get("empresas");
  const inicio = p.get("inicio") ?? "";
  const fim = p.get("fim") ?? "";
  const qc = useQueryClient();

  const { sel, mudar, filtrada, qs: qsSel } = useSelecaoPessoal(empresa);
  ...
  const opcoes = useConsulta<FolhaFiltros>("folha-filtros", qs == null ? null : `/api/folha/filtros?${qs}`);
  const turnover = useConsulta<TurnoverResp>(
    "folha-turnover",
    qsTela == null ? null : `/api/folha/turnover?${qsTela}`
  );

  const [vista, setVista] = useEstadoTela<VistaPessoas>("vista-pessoas", "todos");
  const urlMov = (efetivo: boolean) =>
    qsTela == null ? null : `/api/folha/movimentacoes?${qsTela}${efetivo ? "&escopo=efetivo" : ""}`;
  ...
  const movimentos = useConsulta<FolhaMovimentacao[]>("folha-movimentacoes", urlMov(vistaEhEfetivo(vista)), {
  ...
  const nomeEmpresa = empresas.data?.find((e) => String(e.codigo) === empresa)?.nome ?? `Empresa ${empresa ?? ""}`;
  ...
  const cortes = useMemo<CorteExportar[]>(() => {
    ...
    const sufixo = `${empresa}-${inicio}_${fim}${filtrada ? "-filtrado" : ""}`;
    const pessoas =
      (efetivo: boolean) =>
      async () => {
        const url = `/api/folha/movimentacoes?${qsTela}${efetivo ? "&escopo=efetivo" : ""}`;
        // A mesma chave da lista na tela: se ela já veio, o arquivo sai do cache.
        const linhas = await qc.ensureQueryData({
          queryKey: ["folha-movimentacoes", url],
    ...
    return lista;
  }, [d, qsTela, empresa, inicio, fim, filtrada, qc]);
  ...
  const acoes = (
    <AcoesPagina>
      <MenuExportar modulo="folha" cortes={cortes} imprimir="rotatividade" desabilitado={carregando} />
    </AcoesPagina>
  );
        ...
        <div className="nx-vidro rounded-painel">
          {filtrada ? (
            ...
            <Vazio
              ...
              descricao="A empresa não tem contrato ativo, admitido ou desligado no período. Confira a empresa e o período no topo."
            />
          )}
        </div>
      </>
  ...
  return (
    <>
      ...
      <CabecalhoPapel
        titulo="Rotatividade"
        empresa={{ codigo: Number(empresa), nome: nomeEmpresa }}
        itens={[
      ...
      {QUEBRAS.filter((q) => !q.compacto).map((q) => (
        <PainelQuebraTurnover
          ...
          grupos={d?.[q.chave]}
          total={c}
          rotuloTotal={filtrada ? "Total do recorte" : "Total da empresa"}
          carregando={carregando}
      ...
      <ModalFicha modulo="folha" pessoa={aberta} onFechar={() => setAberta(null)} />
      <ModalPessoas modulo="folha" qs={qs ?? ""} sel={sel} drill={drill} onFechar={() => setDrill(null)} />
    </>
  );
}
