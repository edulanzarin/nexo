...
export default async function LayoutTi({ children }: { children: React.ReactNode }) {
  ...
  const sessao = await assertAcesso("ti");
  return (
    <ProvedorCasca dados={dadosCasca(sessao)}>
      <Suspense>
        <MolduraModulo moduloId="ti">{children}</MolduraModulo>
      </Suspense>
    </ProvedorCasca>
  );
}
