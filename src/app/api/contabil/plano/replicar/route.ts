...
/**
 * As duas pontas precisam estar no escopo da sessão: o preview lê o plano da
 * origem e a execução grava no destino (furo herdado do nexo2, onde bastava ter
 * a seção para copiar de e para qualquer empresa).
 */
async function assertEscopo(origem: number, destino: number): Promise<void> {
  await assertEmpresaVisivel(origem);
  await assertEmpresaVisivel(destino);
}
...
export const GET = apiRoute(async (req) => {
  ...
  await assertEscopo(origem, destino);
