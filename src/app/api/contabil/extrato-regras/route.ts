...
export const DELETE = apiRoute(async (req) => {
  ...
  // Apagar pelo id sem olhar a dona deixava qualquer sessão com a seção apagar
  // regra de empresa fora do próprio escopo (furo herdado do nexo2).
  const dona = await empresaDaRegra(id);
  if (dona == null) return { ok: false };
  await assertEmpresaVisivel(dona);
